...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
  ...
  void _onStart() async {
    ...
    final getUser = await userRepository.fetchUserInformation();

    log("NAME ->${getUser?.firstName} ${getUser?.middleName} ${getUser?.lastName}");
    log("EMAIL ->${getUser?.emailAddress}");
    log("DEVICE_ID ->${getUser?.deviceId}");

    if (getUser != null) {
      setState(() {
        id = getUser.id;
        name = "${getUser.firstName} ${getUser.middleName} ${getUser.lastName}";
      });
    }
  }
